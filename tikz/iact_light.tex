...
% \pagecolor{darkmode}
    \begin{tikzpicture}
      ...
      \draw[maincolour] (-7, 1) -- (14, 1); % name path=horizontalLine
      % \tikzfillbetween[of=A and B]{blue, opacity=0.1};

      \node[] at (8,2.3) {\reflectbox{\includegraphics[height=0.4cm]{build/tikz/xkcd_light.pdf}}};


      \fill[opacity=0.7, path fading=fade30, left color=cherenkov_blue, right color=cherenkov_blue] (0,8) -- (3,0) -- (9,0) -- cycle;
      \draw[warmyellow] (-0.65,9) -- (0.1,7.85) node[midway, above right, maincolour] {$\gamma$};
      \fill[opacity=1, path fading=fadeEAS, left color=warmyellow, right color=warmyellow] (0,8) -- (1,6) arc(30:200:-0.35) -- cycle;
      ...
      \path[maincolour, every node/.style={font=\sffamily\small}, ->, >=latex] (5,5) edge[bend right] node [left] {} (4,4.445);
      \node[maincolour, right] at (5,5) {\footnotesize Pool of Cherenkov Light};

      \path[maincolour, every node/.style={font=\sffamily\small}, ->, >=latex] (2,7) edge[bend right] node [left] {} (1.3,6.5);
      \node[maincolour, right] at (2,7) {\footnotesize Extensive Air Shower (EAS)};

      \node [circle, draw, color=maincolour, fill=darkmode, minimum width = 2.5cm, path picture = {
        \node [] at (path picture bounding box.center) {\includegraphics[height=2.8cm]{graphics/mst_camera_frame.pdf}};
        }] (CAMERAFRAME) at (-0.5,2) {};

      \path[maincolour, every node/.style={font=\sffamily\small}, ->, >=latex] (3.2,2.6) edge[bend right] (CAMERAFRAME);
